module Duss
  class Client
    ...
    def shorten(url)
      ...
      raise JSON.parse(@response.body)['message'] if @response.status != 201
      @response.body
    end
    ...
    def add_header(key, value)
      previous_headers = connection_object.headers
      connection_object.headers = previous_headers.merge("#{key}": value)
    end
  end
end
